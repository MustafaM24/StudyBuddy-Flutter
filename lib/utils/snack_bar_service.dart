...
class SnackBarService {
  static void showConfirmationSnackBar(BuildContext context, [String? text]) {
    showSnackBar(context, text ?? "Something went wrong. Please try again later.", const Icon(Icons.check, color: Colors.white), backgroundColor: Theme.of(context).primaryColor);
  }

  static void showWarningSnackBar(BuildContext context, [String? text]) {
    showSnackBar(context, text ?? "Something went wrong. Please try again later.", const Icon(Icons.warning, color: Colors.yellow));
  }
